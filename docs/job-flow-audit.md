...
| `/api/jobs/[jobId]/enqueue` | `${API_BASE_URL}/jobs/:jobId/enqueue` | `POST` | **Request**: no body. **Response**: upstream job payload. | Returns `500` if `API_BASE_URL` missing. Uses proxy passthrough for upstream errors. | UI allowed enqueue when target was only drafted (not confirmed). |
...
2. **Start analysis gating**: enqueue is now blocked unless `target.confirmed === true`.
3. **Frame mapping robustness**: target selections now preserve `frame_key` and time fields to avoid float mismatches when resolving frames.
